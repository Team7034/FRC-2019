...
public class RocketFinder extends Command {
  ...
  public double phi; //angle that we want to actually turn
  ...
  protected void execute() {
    ...
    SmartDashboard.putNumber("current angle", Robot.m_driveTrain.gyro.getYaw());
      ...
      if (turn > 0.5)
        ...
        turn = -0.5;
      if (current_angle - target <= 20 && current_angle -target >= 5)
        turn = 0.3;
      else if (-current_angle + target >= 20 && -current_angle + target <= 5)
        turn = -0.3;
