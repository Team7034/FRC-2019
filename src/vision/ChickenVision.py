...
#lower_green = np.array([40.467625899280584,66.50179856115108,130.46942446043167])
#upper_green = np.array([136.9965870307167, 255, 250])
...
def findTargets(frame, mask):
    ...
    h_p = 0
    if len(contours) != 0:
        image, h_p = findTape(contours, image, *getCenter())
    # Shows the contours overlayed on the original video
    return image, contours, h_p
...
def findCargo(frame, mask):
    ...
    # Processes the contours, takes in (contours, output_image, (centerOfImage)
    if len(contours) != 0:
        image = findBall(contours, image, *getCenter())
    # Shows the contours overlayed on the original video
    return image
...
def findTape(contours, image, centerX, centerY):
    ...
    if len(contours) >= 2:
        ...
        for cnt in cntsSorted:
            ...
            if (checkContours(cntArea, hullArea)):
                ...
                if(len(biggestCnts) < 13):
                    ...
                    # Creates box around that rectangle
                    box = cv2.boxPoints(rect)
                    # Not exactly sure, list of points
                    box = np.int0(box)
                    # Draws rotated rectangle
                    cv2.drawContours(image, [box], 0, (23, 184, 80), 3)
        ...
        for i in range(len(biggestCnts) - 1):
            ...
            if (np.sign(tilt1) != np.sign(tilt2)):
                ...
                centerTargetY = math.floor((cy1 + cy2) / 2)
                ...
                if not targets:
                    targets.append([centerOfTarget, yawToTarget, centerTargetY])
                    #targets.append([centerOfTarget, yawToTarget])
                elif [centerOfTarget, yawToTarget, centerTargetY] not in targets:
    ...
    if (len(targets) > 0):
        ...
        cy = finalTarget[2]
        h_p = 82.5 - cy #82.5 is magic number. see parker's phone
        # pushes vision target angle to network tables
        networkTable.putNumber("tapeYaw", currentAngleError)
        networkTable.putNumber("height_pixels", h_p)
        
        offset = math.fabs(distance * math.cos(currentAngleError))
        networkTable.putNumber('offset', offset)
    else:
...
def calculateDistance(cameraHeight, targetHeight, h_p):
    '''
    ...
    d = h / tan a
              ...
              camera -----
                       d
    '''
    # pitch = calculatePitch(pixelY=0, centerY=0 vFocalLength=0)
    '''pitch = networkTable.getNumber("pitch", 1)
    ...
    #d = Tft*FOVpixel/(2*Tpixel*tan0)
    #Tft/Tpixel = FOVft/FOVpixel
    #FOVft = 2*w = 2*d*tan0
    #more info at https://wpilib.screenstepslive.com/s/currentCS/m/vision/l/288985-identifying-and-processing-the-targets
    '''cntArea = cv2.contourArea(cnt)
    width = image_width / 2
    Tft = cntArea
    Tpixel = Tft*96
    ...
    FOVpixel = FOVft*96
    Theta = 78/2
    denom = (2*Tpixel*math.tan(Theta))
    if denom != 0:
        return Tft*FOVpixel/denom
    else:
        return -1'''
    #return (h_p * (1 / ((.0225 * h_p)-.34125)))
    return (h_p * (1 / ((1 * h_p)-.34125)))



    # start NetworkTables
    # ntinst = NetworkTablesInstance.getDefault()
    # networkTable = ntinst.getTable('ChickenVision')

    # #These three lines are not in the original
    # #Gets the offset using trig
    # # Updates the network tables for the offset
    # networkTable.putNumber("Offset", Offset)
    # #updates the network tables for the distance
    # networkTable.putNumber("Distancetotarget", distance)
...
def getEllipseRotation(image, cnt):
    try:
        ...
        centerE = ellipse[0]
        # Gets rotation of ellipse; same as rottation of contour
        rotation = ellipse[2]
    ...
    while True:
        ...
        if(networkTable.getBoolean("Driver", False)):
            ...
            if(networkTable.getBoolean("Tape", True)):
                ...
                processed, cnts, h_p = findTargets(frame, threshold)

                # TODO Replace with actual values, these are fake test values

                # Update the distance from the target
                distance = calculateDistance(12, 36, h_p)
                
                # offset = math.fabs(distance * math.cos(currentAngleError))          
                networkTable.putNumber("Distance from Target", distance)
            else:
        ...
        networkTable.putNumber("Distance to Target", distance)
